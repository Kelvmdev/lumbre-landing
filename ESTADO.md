...
`estado: 🟢 publicada, backlog 🟢 y 🟡 COMPLETOS + agentes/llms.txt OK; 🔴 #13 CMS full EN PROGRESO (Fases 1a+1b+2 hechas y pushed)` · `en vivo: https://lumbre-landing.vercel.app`

> ⏭️ **ARRANCAR AQUÍ (próxima sesión):** CMS **Fase 3** = panel de edición en `/admin` + `api/guardar.ts` (GitHub Contents API), con blindaje del guardado. Detalle en §#13. **ANTES**: (1) Kervin carga las 8 env vars en **Vercel** para que `/admin` viva en prod (local ya funciona); (2) decidir si el contacto (enlaces.ts) se mueve a `data.json`.
...
13. **CMS propio** (tema 04) — un solo `data.json` + Contents API. Fases: (1) capa de datos + cablear páginas sin cambio visual · (2) `/admin` login cookie HMAC + `api/login` · (3) panel edición + `api/guardar` (GitHub Contents API) · (4) subida fotos Cloudinary · (5) probar ciclo real + blindaje guardado.
    ...
    - ⏳ **Fase 3 (SIGUE)**: panel de edición en `/admin` (form por sección leyendo `contenido`) + `api/guardar.ts` (GitHub Contents API: GET sha→PUT base64 UTF-8, User-Agent obligatorio). Blindaje del guardado (partir de `{...original}`, merge por sección). Ojo path: el panel escribe `src/data.json` y las páginas leen ESE mismo (ya es así vía `contenido.ts`).

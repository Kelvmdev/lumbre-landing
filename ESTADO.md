...
`estado: 🟢 publicada, backlog 🟢 y 🟡 COMPLETOS; 🔴 #13 CMS: Fases 1-2 + 3(texto) HECHAS y PROBADAS EN PROD. Falta 3c(listas)+4(Cloudinary)+5` · `en vivo: https://lumbre-landing.vercel.app` · `panel: /admin (vars ya en Vercel)`

> ⏭️ **ARRANCAR AQUÍ (próxima sesión):** CMS **Fase 3c** = editar las LISTAS en el panel (domos, galería, faq, experiencia-features) con UI agregar/quitar fila, + **Fase 4** subida de fotos a Cloudinary (el campo `img` de domos/galería). Conviene hacerlas juntas. Detalle en §#13. **REGLA:** `git pull --no-rebase --no-edit` ANTES de tocar código (el CMS edita el data.json en GitHub). **Pendiente decidir:** mover contacto (enlaces.ts) a data.json.
...
13. **CMS propio** (tema 04) — un solo `data.json` + Contents API. Fases: (1) capa de datos + cablear páginas sin cambio visual · (2) `/admin` login cookie HMAC + `api/login` · (3) panel edición + `api/guardar` (GitHub Contents API) · (4) subida fotos Cloudinary · (5) probar ciclo real + blindaje guardado.
    ...
    - ⏳ **Fase 3c (SIGUE) — LISTAS**: falta editar en el panel los arrays: `domos` (nombre/img/alt/capacidad/precio/detalles[]), `galeria` (5 fotos), `faq.preguntas` (q/a), `experiencia.features` (titulo/desc/icono SVG). Necesitan UI agregar/quitar fila. La imagen (campo `img`) conecta con **Fase 4 (Cloudinary)** — la subida de fotos. Sugerencia: hacer arrays + subida juntas.
